...
"""Tests for the JointDistributionEdward2."""
...
class JointDistributionEdward2Test(tf.test.TestCase):
  ...
  def test_batch_and_event_shape_no_plate(self):
    ...
    def dist():
      a = ed.Bernoulli(probs=0.5, dtype=tf.float32)
      b = ed.Bernoulli(probs=0.25 + 0.5*a, dtype=tf.float32)
      ed.Normal(loc=a, scale=1. + b)

    joint = ed.JointDistributionEdward2(dist, validate_args=True)
  ...
  def test_batch_and_event_shape_with_plate(self):
    ...
    def dist():
      g = ed.Gamma(2, 2)
      df = ed.Exponential(1.)
      loc = ed.Sample(tfd.Normal(0, g), 20)
      ed.Independent(tfd.StudentT(tf.expand_dims(df, -1), loc, 1), 1)
  ...
  def test_log_prob_with_plate(self):
    ...
    def dist():
      a = ed.Bernoulli(probs=0.5, dtype=tf.float32)
      b = ed.Sample(tfd.Bernoulli(probs=0.25 + 0.5*a, dtype=tf.float32), 2)
      ed.Independent(tfd.Normal(loc=a, scale=1. + b), 1)
  ...
  def test_log_prob_multiple_roots(self):
    ...
    def dist():
      a = ed.Exponential(1.)
      b = ed.Sample(tfd.Normal(a, 1.), 20)
      c = ed.Exponential(1.)
      ed.Independent(tfd.Normal(b, tf.expand_dims(c, -1)), 1)
  ...
  def test_sample_dtype_structures_output(self):
    def noncentered_horseshoe_prior(num_features):
      scale_variance = ed.InverseGamma(0.5, 0.5)
      scale_noncentered = ed.Sample(tfd.HalfNormal(1.), num_features)
      scale = scale_noncentered * scale_variance[..., None]**0.5
      weights_noncentered = ed.Sample(tfd.Normal(0., 1.), num_features)
      ed.Independent(tfd.Deterministic(weights_noncentered * scale),
    ...
    joint = ed.JointDistributionEdward2(
    ...
    self.assertEqual([4], joint.log_prob(joint.sample([4])).shape)

  def test_repr_with_custom_sample_dtype(self):
    def model():
      s = ed.Sample(tfd.InverseGamma(2, 2), 100)
      ed.Independent(tfd.Normal(0, s), 1)
    sd = collections.namedtuple('Model', ['s', 'w'])(None, None)
    m = ed.JointDistributionEdward2(model, sample_dtype=sd)
    self.assertEqual(
        ('tfp.distributions.JointDistributionEdward2('
