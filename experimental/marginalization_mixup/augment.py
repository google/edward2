...
def transform(image, transforms):
  """Prepares input data for `tf._keras_internal.layers.preprocessing.image_preprocessing.transform`."""
  original_ndims = tf.rank(image)
  transforms = tf.convert_to_tensor(transforms, dtype=tf.float32)
  if len(tf.shape(transforms)) == 1:
    transforms = transforms[None]
  image = to_4d(image)
  image = tf._keras_internal.layers.preprocessing.image_preprocessing.transform(
      images=image,
